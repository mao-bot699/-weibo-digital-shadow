...
基于公开微博数据，AI 驱动的个人数字画像分析工具。

输入一个微博 UID，自动获取用户公开数据，通过 LLM 生成多维度的深度画像报告，包括性格特征、生活方式、社交关系、兴趣领域等。

## 功能特性

- **用户搜索**：通过微博昵称搜索用户（需有效 Cookie）
- **UID 直查**：直接输入 UID 获取用户信息，更稳定
- **深度画像报告**：AI 生成 10 大维度分析报告（2000+ 字）
...
5. 社交网络特征（粉丝互动、转发原创比例、影响力层级）
6. 互动关系图谱（高频 @对象、社交圈层、互动模式）
7. 多平台线索分析（跨平台账号线索、可信度评估）
8. 语言与表达特征（语言风格、网络流行语、标点 emoji 习惯）
9. 数据时间线（发博规律、活跃期/沉默期、主题演变）
10. 综合总结（核心特征、三个关键词、出人意料的发现）
...
```bash
git clone https://github.com/<your-username>/数字影子.git
cd 数字影子
